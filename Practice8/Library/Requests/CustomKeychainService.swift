//
//  CustomKeychainService.swift
//  Carduelis
//
//  Created by Andrew on 08.10.2021.
//

import Foundation

open class CustomKeychainService {
    ...
    open func save (data: Data,
                     toService service: String = CommonFunctions.Application.bundleIdentifier ?? String.random(),
                     account: String) throws {

        do {
            try self.executeUpdate(data: data,
                                    service: service,
                                    account: account)
        } catch let error {
    ...
    open func read(fromService service: String = CommonFunctions.Application.bundleIdentifier ?? String.random(),
                     account: String) throws -> Data {
        return try self.executeRead(service: service,
                                account: account)
    }


    open func delete(fromService service: String = CommonFunctions.Application.bundleIdentifier ?? String.random(),
                       account: String) throws {
        try self.executeDelete(service: service,
                               account: account)
    }

}

extension CustomKeychainService {

    private func executeAdd(data: Data,
                             service: String,
                             account: String) throws {
    ...
    private func executeUpdate(data: Data,
                               service: String,
                               account: String) throws {
    ...
    private func executeRead(service: String,
                            account: String) throws -> Data {
        let query: [String: AnyObject] = [
            // kSecAttrService,  kSecAttrAccount, and kSecClass
    ...
    private func executeDelete(service: String,
                               account: String) throws {
        let query: [String: AnyObject] = [
            // kSecAttrService,  kSecAttrAccount, and kSecClass
